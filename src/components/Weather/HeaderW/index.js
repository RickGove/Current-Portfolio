...
class HeaderW extends React.Component {
	...
	async getTimeAtSearch() {
		if (this.props.searchWeatherData.data !== undefined) {
			...
			const site = `http://worldtimeapi.org/api/timezone/${timeZone}`;
			axios
				.get(site)
				.then((response) => {
	...
	async findWeatherInfo(latLon) {
		...
		let site = `http://api.openweathermap.org/data/2.5/weather?lat=${latLon[0]}&lon=${latLon[1]}&appid=${key}`;
		axios
			.get(site)
			.then((response) => {
	...
	loadIcon() {
		if (this.props.location[3] !== '') {
			return `http://openweathermap.org/img/wn/${this.props.location[3]}.png`;
		}
	}
	...
	searchLocation = (c) => {
		let key = `f63ee05c044c91f80348c4e021c7d476`;
		let coord = ['lat', 'lon'];
		let site = `http://api.openweathermap.org/data/2.5/weather?q=${c}&appid=${key}`;
		axios
			.get(site)
			.then((response) => {
